...
*   `port`: Порт, на кvотором работает сервис.
...
*   `smart_process_settings`: Структура настроек полей для каждого типа смарт-процесса, где ключ это ID типа смарт-процесса, а значение структура описывающая необходимые для работы скрипта поля (таки как deals_uf, file_uf и пр.) и которых будет взята информация.
    *   `deals_uf`: Поле смарт-процесса с множественным выборов, в котором указаны сделки.
    *   `file_uf`: Поле смарт-процесса, для размещения файла отчета.
    *   `deals_fields_for_report`: Массив полей сделки, для вывода в отчет Excel (как стандартыне поля, так и пользовательские UF_CRM_).
